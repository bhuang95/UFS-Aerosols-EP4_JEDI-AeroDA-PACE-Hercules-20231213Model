...
VarDims = {'aerosolOpticalDepth': ['Location', 'Channel'],
           ...
           'fineModeAerosolOpticalDepth':['Location', 'Channel'],
...
class AOD(object):
    ...
    def _read(self):
        ...
        for f in self.filenames:
            ...
            valtmp=ncd.groups['geophysical_data'].variables['Aerosol_Optical_Depth'][:,:,:]
            ...
            if nlocs1 > 0 :
                ...
                for iodavar in obsvars:
                    if iodavar == "aerosolOpticalDepth":
                    ...
                    if iodavar == "absorptionAerosolOpticalDepth":
                        self.outdata[self.varDict[iodavar]['valKey']] = np.append(
                            self.outdata[self.varDict[iodavar]['valKey']], np.array(vals*0.1, dtype=np.float32))
                        self.outdata[self.varDict[iodavar]['errKey']] = np.append(
                            self.outdata[self.varDict[iodavar]['errKey']], np.array(vals*0.1, dtype=np.float32))
                    if iodavar == "fineModeAerosolOpticalDepth":
                        self.outdata[self.varDict[iodavar]['valKey']] = np.append(
                            self.outdata[self.varDict[iodavar]['valKey']], np.array(vals*0.7, dtype=np.float32))
                        self.outdata[self.varDict[iodavar]['errKey']] = np.append(
                            self.outdata[self.varDict[iodavar]['errKey']], np.array(vals*0.7, dtype=np.float32))
