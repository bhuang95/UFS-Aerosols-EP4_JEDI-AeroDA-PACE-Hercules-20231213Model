...
class MidpointNormalize(mpcrs.Normalize):
    def __init__(self, vmin=None, vmax=None, midpoint=None, clip=False):
        self.midpoint = midpoint
        mpcrs.Normalize.__init__(self, vmin, vmax, clip)
...
def plot_map_satter_aod_hfx(lons, lats, obs, hfx, hfx2obs, cmap_aod, cmap_bias, titlepre, cycpre):
    ...
    for ipt in range(3):
        ...
        if ipt==0:
            data=obs
            tstr=f"(a) PACE OCI AOD at {cycpre}"
            vvend=vvend1
            cmap=ccmap1
            norm=norm1
        elif ipt==1:
            ...
            ########
            ####
            norm = MidpointNormalize( vmin=-0.8, vmax=0.25, midpoint = 0.0, clip=True )
            cmap = 'seismic'
            ####
        ...
        if ipt==100:
            ...
            x,y=map(lons, lats)
            cs=map.scatter(lons,lats, s=0.001, c=data, marker='.', cmap=cmap, norm=norm, alpha=0.5)
            #cs=map.scatter(lons,lats, s=0.05, c=data, marker='.', cmap=cmap)
    ...
    fig.tight_layout()
    pname = f"{titlepre}-{cycpre}-relativeBias.png"
...
if __name__ == '__main__':
    ...
    for trcr in trcrs:
        ncpre = f"NASA_PACE_OCI_obs_hofx_3dvar_LUTs_{trcr}"
        ...
        while cyc < cyced:
            ...
            with nc.Dataset(ncfile, 'r') as ncdata:
                ...
                if cyc == cycst:
                    ...
                    qc = np.concatenate((qc, qctmp), axis=0)
            cyc = ndate(cyc, 6)

        for ichan in range(len(chans)):
            wav = wavs[ichan]
            validind = np.where((qc[:, ichan]==0) & (obs[:, ichan] >= 0.02))
            lon_in = lon[validind]
            lat_in = lat[validind]
            obs_in = obs[validind, ichan]
            hfx_in = hfx[validind, ichan]
            hfx2obs_in = (hfx_in - obs_in)/obs_in
            titlepre_in = f"{titlepre}-Hofx-Diff-{wav}"
            plot_map_satter_aod_hfx(lon_in, lat_in, obs_in, hfx_in, hfx2obs_in, cmap_aod, cmap_bias, titlepre_in, cycpre)
