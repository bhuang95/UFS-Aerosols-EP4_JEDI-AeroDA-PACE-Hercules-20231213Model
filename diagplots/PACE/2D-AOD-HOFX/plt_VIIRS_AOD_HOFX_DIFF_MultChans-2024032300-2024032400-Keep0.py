...
def plot_map_satter_aod_hfx(lons, lats, obs, hfx, hfx2obs, cmap_aod, cmap_bias, titlepre, cycpre):
    ...
    for ipt in range(3):
        ...
        if ipt==0:
            ...
            #        bokeh.palettes.Plasma[ncolors],
            #        bokeh.palettes.Viridis[ncolors],
            #        n=ncolors,
            ...
            ########
            ####
            norm = MidpointNormalize( vmin=-0.4, vmax=0.05, midpoint = 0.0, clip=True )
        ...
        if ipt==100:
            ...
            x,y=map(lons, lats)
            cs=map.scatter(lons,lats, s=0.01, c=data, marker='.', cmap=cmap, norm=norm)
...
if __name__ == '__main__':
    ...
    for trcr in trcrs:
        ...
        for ichan in range(len(chans)):
            wav = wavs[ichan]
            validind = np.where(qc[:, ichan]==0)
            lon_in = lon[validind]
            lat_in = lat[validind]
            obs_in = obs[validind, ichan]
            hfx_in = hfx[validind, ichan]
            hfx2obs_in = hfx_in - obs_in
